...
export const buildPrDependencyTreeFooter = (
	tree: string,
	showPancakeCredit: boolean,
): string => {
	const body = `${footerTitle}${tree}`;
	return showPancakeCredit ? `${body}${footerFooter}` : body;
};
...
export function createPrBodyFooter(context: TContext, branch: string): string {
	...
	const showPancakeCredit =
		context.userConfig.data.submitPancakeAttribution ?? true;

	return buildPrDependencyTreeFooter(tree, showPancakeCredit);
}
